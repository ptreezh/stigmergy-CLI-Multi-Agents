...
async function scanProjectSessions(query, context) {
  const projectPath = 'D:\stigmergy-CLI-Multi-Agents\test-resumesession-fix';
  const allSessions = [];

  // 扫描各个CLI的会�?  const cliPaths = {
  ...
  // 应用过滤�?  let filteredSessions = allSessions;
  ...
  // 按时间排序（最新的在前�?  filteredSessions.sort((a, b) => new Date(b.updatedAt) - new Date(a.updatedAt));
    ...
    for (const file of files) {
      if (file.endsWith('.json') || file.endsWith('.session')) {
        try {
          ...
          // 检查是否属于当前项�?          if (isProjectSession(session, projectPath)) {
            sessions.push({
              cliType,
...
function isProjectSession(session, projectPath) {
  ...
  if (!sessionProject) return true; // 如果没有项目路径，假设属于当前项�?
