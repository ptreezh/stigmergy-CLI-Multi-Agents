...
class EnhancedUninstaller {
  ...
  async completeUninstall() {
    console.log('🗑️  Starting Enhanced Stigmergy Uninstall...\n');
  ...
  async createUninstallPlan() {
    console.log('📋 Creating Uninstall Plan...\n');
  ...
  async cleanStigmergyDirectory() {
    console.log('📁 Cleaning Stigmergy directory...');

    if (!fs.existsSync(this.stigmergyDir)) {
      console.log('  ℹ️  Stigmergy directory not found');
      return;
    }
    ...
    if (this.options.dryRun) {
      console.log(`  🔍 Would remove ${files.length} files and directories`);
      this.logFiles(files, '  ');
      return;
    }
    ...
    try {
      ...
      console.log(`  ✅ Removed ${files.length} files and directories`);
      this.results.filesRemoved += files.length;
      this.results.directoriesRemoved++;
    } catch (error) {
      console.error(`  ❌ Failed to remove Stigmergy directory: ${error.message}`);
  ...
  async cleanTestDirectory() {
    ...
    if (!fs.existsSync(this.stigmergyTestDir)) {
      console.log('  ℹ️  Test directory not found');
      return;
    }

    if (this.options.dryRun) {
      console.log('  🔍 Would remove test directory');
      return;
    }
  ...
  async cleanCLIConfigurations() {
    ...
    for (const cli of this.supportedCLIs) {
      ...
      try {
        ...
        console.log(`    ✅ Cleaned ${stigmergyFiles.length} files`);
      } catch (error) {
        console.error(`    ❌ Failed to clean ${cli}: ${error.message}`);
  ...
  async cleanNPXCache() {
    ...
    console.log(`  📦 Found ${npxCacheDirs.length} Stigmergy entries in NPX cache`);
    ...
    for (const cacheDir of npxCacheDirs) {
      try {
        ...
        console.error(`    ❌ Failed to remove ${cacheDir}: ${error.message}`);
  ...
  async cleanTemporaryFiles() {
    ...
    for (const tempDir of tempDirs) {
      ...
      try {
        ...
        if (this.options.dryRun) {
          console.log(`  🔍 ${tempDir}: ${tempFiles.length} temporary files`);
          continue;
        }
        ...
        if (tempFiles.length > 0) {
          console.log(`  ✅ ${path.basename(tempDir)}: removed ${tempFiles.length} files`);
        }

      } catch (error) {
        console.error(`  ❌ Failed to clean ${tempDir}: ${error.message}`);
    ...
    for (const pkg of globalPackages) {
      try {
        ...
        if (result.status === 0 && result.stdout.includes(pkg)) {
          console.log(`  📦 Found global package: ${pkg}`);
          ...
          if (uninstallResult.status === 0) {
            ...
            console.log(`    ⚠️  Failed to uninstall: ${pkg}`);
          }
        }
      } catch (error) {
