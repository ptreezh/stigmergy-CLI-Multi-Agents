...
const os = require("os");
const { SkillParser } = require("./SkillParser");
...
class SkillInstaller {
  ...
  parseGitHubUrl(url) {
    ...
    // 所有尝试都失败，提供详细的错误信息
    const attemptedFormats = attempts
  ...
  async installSkill(skill, force = false) {
    ...
    try {
      ...
      if (!force) {
        throw new Error(
          `Skill '${skill.name}' already exists. Use --force to overwrite.`,
        );
      }
      // Remove existing
  ...
  async installFromRepository(urlInfo, options = {}) {
    ...
    const tempDir = path.join(
      os.tmpdir(),
      `stigmergy-skill-install-${Date.now()}`,
    );
    ...
    try {
      ...
      console.log(
        `[SUCCESS] 成功安装 ${installed.length}/${skills.length} 个技能`,
      );
      return installed;
    } catch (error) {
      // 提供友好的错误信息
  ...
  async installFromRawFile(urlInfo, options = {}) {
    ...
    console.log(
      `[INFO] 从 raw 文件安装: ${owner}/${repo}/${branch}/${filePath}`,
    );
  ...
  async installFromFileOrDirectory(urlInfo, options = {}) {
    ...
    console.log(
      `[INFO] 从仓库路径安装: ${owner}/${repo}/${branch}/${filePath}`,
    );
