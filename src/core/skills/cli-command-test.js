...
async function runCLITests() {
  ...
  await runner.test("list command output format is correct", async () => {
    ...
    try {
      ...
      assert(
        output.includes("cli-test-skill") || output.includes("No skills"),
  ...
  await runner.test(
    "install command errors when parameters missing",
    async () => {
      ...
      const hasError =
        errorThrown || runner.errors.some((e) => e.includes("source required"));
  ...
  await runner.test("unknown command errors correctly", async () => {
    ...
    const hasError =
      errorThrown || runner.errors.some((e) => e.includes("Unknown"));
  ...
  await runner.test("remove command alias rm works", async () => {
    ...
    const hasParamError = runner.errors.some((e) =>
      e.includes("skill name required"),
    );
