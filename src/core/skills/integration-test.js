...
class IntegrationTestRunner {
  constructor() {
    this.testDir = path.join(
      os.tmpdir(),
      `stigmergy-integration-test-${Date.now()}`,
    );
...
async function runIntegrationTests() {
  ...
  try {
    ...
    await runner.test("Scan local skill repository", async () => {
      ...
      assert(
        skills.some((s) => s.name === "test-analyzer"),
        "Missing test-analyzer",
      );
      assert(
        skills.some((s) => s.name === "test-formatter"),
        "Missing test-formatter",
      );
    });
    ...
    await runner.test("Read installed skill", async () => {
      ...
      assert(
        ...
        `Expected name 'test-analyzer', got '${skill.name}'`,
      );
      assert(
        skill.content.includes("Test Analyzer"),
    ...
    await runner.test("Sync skills to AGENTS.md", async () => {
      ...
      try {
        ...
        assert(
          content.includes("test-analyzer"),
          "Missing test-analyzer in AGENTS.md",
        );
      } finally {
    ...
    await runner.test("Remove skill", async () => {
      ...
      const exists = await fs
        .access(path.join(runner.testDir, "skills", "test-analyzer"))
        .then(() => true)
        .catch(() => false);
