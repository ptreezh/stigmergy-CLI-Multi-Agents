    ...
    try {
      ...
      await runner.test("Sync to all CLI config files", async () => {
        ...
        for (const file of cliFiles) {
          const exists = await fs
            .access(path.join(runner.testDir, file))
            .then(() => true)
            .catch(() => false);
      ...
      await runner.test("Verify skill content correctly inserted", async () => {
        ...
        for (const file of cliFiles) {
          const content = await fs.readFile(
            path.join(runner.testDir, file),
            "utf-8",
          );
          assert(
            content.includes("<!-- SKILLS_START -->"),
            `${file} missing skills start marker`,
          );
          assert(
            content.includes("<!-- SKILLS_END -->"),
            `${file} missing skills end marker`,
          );
          assert(
            content.includes("<available_skills>"),
            `${file} missing skills section`,
          );
      ...
      await runner.test("Verify sync operation idempotency", async () => {
        ...
        assert(
          ...
          "Multiple syncs produced different results",
        );
      });

      // Test 5: Verify newly created file format
      await runner.test(
        "Verify newly created file format is correct",
        async () => {
          const newFiles = [
            "gemini.md",
            "iflow.md",
          ...
          for (const file of newFiles) {
            ...
            assert(
              content.includes(`# ${cliName} Configuration`),
              `${file} missing header`,
            );
          }
        },
