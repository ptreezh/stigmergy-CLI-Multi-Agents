...
class ComprehensiveE2ETestRunner {
  ...
  summary() {
    ...
    for (const [category, stats] of Object.entries(byCategory)) {
      console.log(
        `   ${category}: [OK]${stats.passed} [X]${stats.failed} [INFO]${stats.skipped}`,
      );
    }
...
async function runComprehensiveTests() {
  ...
  console.log(
    "Test Scope: Cache Clean -> Install -> Command Test -> Cross-Call -> Skill Usage\n",
  );
  ...
  await runner.test(
    "stigmergy --version",
    () => {
      const output = runner.exec("node bin/stigmergy --version", {
        silent: true,
      });
  ...
  await runner.test(
    "skill (short command - default sync)",
    () => {
  ...
  await runner.test(
    ...
    async () => {
      ...
      const listOutput = runner.exec("node bin/stigmergy skill-l", {
        silent: true,
      });
      ...
      runner.exec(`node bin/stigmergy skill-r ${skillName}`, {
        timeout: 10000,
      });
    },
  ...
  await runner.test(
    ...
    async () => {
      ...
      try {
        ...
        runner.exec(`node bin/stigmergy skill-v ${skillPath}`, {
          timeout: 10000,
          allowFail: true,
        });
      } catch (err) {
      ...
      async () => {
        ...
        try {
          ...
          if (!content.includes("<!-- SKILLS_START -->")) {
            console.log(
              `   [INFO] ${fileName} has no skills section (may not be synced yet)`,
            );
            return;
          }
      ...
      () => {
        try {
          ...
          if (result.status === 0 && result.stdout.trim()) {
            console.log(
              `   [OK] ${tool} installed: ${result.stdout.trim().split("\n")[0]}`,
            );
  ...
  if (availableCLIs.length === 0) {
    ...
    for (const cli of availableCLIs.slice(0, 3)) {
      // Test first 3 available CLIs
      await runner.test(
        `Cross-call ${cli}`,
        () => {
          try {
  ...
  await runner.test(
    ...
    async () => {
      ...
      const readOutput = runner.exec(
        `node bin/stigmergy skill-r ${skillName}`,
        {
          silent: true,
          timeout: 15000,
        },
      ...
      console.log(
        `   [OK] Skill read successfully, content length: ${readOutput.length} characters`,
      );
    },
  ...
  await runner.test(
    "Integration test",
    () => {
      runner.exec("node src/core/skills/integration-test.js", {
        timeout: 60000,
      });
    },
  ...
  const reportPath = path.join(
    runner.projectRoot,
    `comprehensive-e2e-report-${Date.now()}.json`,
  );
  await fs.writeFile(
    reportPath,
    JSON.stringify(
