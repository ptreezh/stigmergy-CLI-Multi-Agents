...
async function runE2ETests() {
  ...
  await runner.test(
    "Scenario 1: Create->Install->Use->Remove complete flow",
    async () => {
...
`,
      ...
      assert(
        allSkills.some((s) => s.name === "e2e-skill"),
        "Skill not in list",
      );
      ...
      const stillExists = await fs
        .access(path.join(runner.testDir, "installed-skills", "e2e-skill"))
        .then(() => true)
        .catch(() => false);
...
`,
    ...
    console.log(
      "  [INFO] Step 2: Agent executes command: stigmergy skill read agent-skill",
    );
    ...
    assert(
      skill.content.includes("Read input file"),
      "Instruction steps missing",
    );
    console.log("  [OK] Agent understood skill instructions");

    // Simulate Agent executing task
    console.log(
      "  [INFO] Step 4: Agent executing task according to skill instructions...",
    );
