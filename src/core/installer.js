...
class StigmergyInstaller extends EnhancedCLIInstaller {
          ...
          if (
            ...
            try {
              ...
              if (jsFileMatch) {
                ...
                const jsFilePath = require("path").join(
                  npmGlobalDir,
                  jsFileMatch[0],
                );
                ...
                if (fsSync.existsSync(jsFilePath)) {
                  ...
                  console.log(
                    "[DEBUG] Codex JS file not found at expected path, will try version check",
                  );
                }
              }
            } catch (scriptError) {
              console.log(
                `[DEBUG] Could not verify codex script: ${scriptError.message}`,
              );
  ...
  async getUserSelection(options, missingTools) {
    ...
    try {
      ...
      console.log(
        "[INSTALL] Interactive mode failed, installing all missing tools...",
      );
      return options.map((option) => option.value);
    }
  }
  ...
  /**
   * Get tools to install including dependencies
   * Handles dependencies like opencode -> oh_my_opencode
   */
  async getToolsWithDependencies(toolNames, missingTools) {
    const toolsToInstall = new Set(toolNames);

    for (const toolName of toolNames) {
      const toolInfo = missingTools[toolName];
      if (toolInfo && toolInfo.dependsOn) {
        for (const dep of toolInfo.dependsOn) {
          if (missingTools[dep]) {
            console.log(`[INFO] Adding dependency: ${dep} for ${toolName}`);
            toolsToInstall.add(dep);
          }
        }
      }
    }

    return Array.from(toolsToInstall);
  }
  ...
  async installTools(selectedTools, missingTools) {
    // 处理依赖关系
    const toolsWithDeps = await this.getToolsWithDependencies(
      selectedTools,
      missingTools,
    );

    // 如果有新增的依赖工具，更新 selectedTools
    if (toolsWithDeps.length > selectedTools.length) {
      console.log(
        `[INFO] Installing ${selectedTools.length} tools + ${toolsWithDeps.length - selectedTools.length} dependencies`,
      );
    }

    // 调用父类的增强安装功能
    const result = await super.installTools(toolsWithDeps, missingTools);
      ...
      for (const toolName of selectedTools) {
        ...
        if (toolInfo && this.results.installations[toolName]?.success) {
          try {
            ...
            console.log(
              `[WARN] Failed to generate hook for ${toolName}: ${error.message}`,
            );
          }
        }
      }
  ...
  async generateToolHook(toolName, toolInfo) {
    ...
    try {
      ...
      console.error(
        `[ERROR] Hook generation error for ${toolName}:`,
        error.message,
      );
      throw error;
    }
  ...
  async deployHooks(available) {
    ...
    console.log(
      chalk.blue(
        `[INFO] Using parallel deployment with concurrency: ${this.concurrency || 6}`,
      ),
    );
    ...
    const deployForTool = async ([toolName, toolInfo]) => {
      ...
      try {
        ...
        if (deploySuccess) {
          ...
          console.log(
            `[OK] ${toolInfo.name} deployed successfully (${duration}ms)`,
          );
    ...
    console.log(
      chalk.blue(`[PERFORMANCE] Total deployment time: ${totalDuration}ms`),
    );
    console.log(
      chalk.blue(
        `[PERFORMANCE] Average per tool: ${Math.round(totalDuration / totalCount)}ms`,
      ),
    );
  ...
  async createProjectFiles() {
    ...
    try {
      ...
      const projectConstitutionPath = path.join(
        currentDir,
        "PROJECT_CONSTITUTION.md",
      );
      if (
        !(await fs
          .access(projectConstitutionPath)
          .then(() => true)
          .catch(() => false))
      ) {
...
`;
    ...
    try {
      ...
      const possiblePaths = [
        ...
        path.join(
          ...
          "adapters",
          adapterDirName,
          `install_${adapterDirName}_integration.js`,
        ),
      ...
      if (!installScriptPath) {
        console.log(
          `[WARN] JavaScript installation script not found for ${toolInfo.name} at possible paths:`,
          possiblePaths,
        );
        console.log(
          `[FALLBACK] Using inline configuration for ${toolInfo.name}...`,
        );
      ...
      console.log(
        `[RUN] Running JavaScript installation script for ${toolInfo.name}...`,
      );
      ...
      if (result.success) {
        console.log(
          `[OK] ${toolInfo.name} installation script completed successfully`,
        );
      } else {
        console.log(
          `[WARN] ${toolInfo.name} installation script failed: ${result.error}`,
        );
        // If the installation script fails but was found, still try to use inline configuration as fallback
        console.log(
          `[FALLBACK] Using inline configuration for ${toolInfo.name} after script failure...`,
        );
        await this.inlineToolConfiguration(toolName, toolInfo);
      }
    } catch (error) {
      console.log(
        `[WARN] Configuration failed for ${toolInfo.name}: ${error.message}`,
      );
    }
  }
  ...
  async inlineToolConfiguration(toolName, toolInfo) {
    ...
    const possibleAdapterPaths = [
      ...
      path.join(
        ...
        "..",
        "adapters",
        toolName === "qodercli" ? "qoder" : toolName,
      ),
    ...
    if (adapterDir) {
      ...
      console.log(
        `[WARN] Adapter directory not found for ${toolInfo.name} at possible paths:`,
        possibleAdapterPaths,
      );
    }
  ...
  async createToolSpecificConfigs(toolName, toolInfo) {
    ...
    try {
      ...
      console.log(
        `[WARN] Failed to create tool-specific configs: ${error.message}`,
      );
    }
  }
  ...
  async createCrossCliDocumentation(toolName, toolInfo) {
    ...
    try {
      ...
      console.log(
        `[WARN] Failed to create Cross-CLI documentation: ${error.message}`,
      );
    }
  }
  ...
  async appendToToolMdFile(toolName) {
    ...
    try {
      ...
      if (exists) {
        ...
        console.log(
          `[OK] Appended Cross-CLI communication guide to ${path.basename(mdFilePath)}`,
        );
      }
    } catch (error) {
