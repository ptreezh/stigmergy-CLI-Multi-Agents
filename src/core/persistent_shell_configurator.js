...
class PersistentShellConfigurator {
  ...
  getShellProfileFile(shellType) {
    ...
    case "wsl":
      ...
      const wslHome =
          process.env.HOME ||
          path.join("/mnt", "c", "Users", process.env.USER || "user");
      const wslShell = process.env.SHELL;
      if (wslShell && wslShell.includes("zsh"))
        return path.join(wslHome, ".zshrc");
      if (wslShell && wslShell.includes("bash"))
  ...
  generatePersistentConfig(npmConfig) {
    ...
    config +=
      "# This configuration enables CLI tools to work from any directory\n\n";
  ...
  async ensureProfileFile(profilePath) {
    try {
      ...
      try {
        ...
        this.log(
          "error",
          `Failed to create profile file: ${createError.message}`,
        );
        return false;
      }
    }
  ...
  async writePersistentConfiguration(npmConfig) {
    ...
    if (!this.results.profileFile) {
      if (this.results.shellType === "cmd") {
        this.log(
          "warn",
          "Command Prompt detected, providing manual instructions",
        );
    ...
    try {
      ...
      const hasExisting = await this.hasExistingConfiguration(
        this.results.profileFile,
      );
      ...
      if (hasExisting && this.options.force) {
        this.log(
          "warn",
          "Overwriting existing configuration due to --force flag",
        );
      ...
      this.log(
        "success",
        `Configuration written to: ${this.results.profileFile}`,
      );
  ...
  async handleCmdPrompt(npmConfig) {
    ...
    this.log(
      "info",
      "Command Prompt detected - providing multiple setup options:",
    );
    ...
    console.log(
      "PowerShell supports profile files for permanent configuration",
    );
...
`;
  ...
  async removeExistingConfiguration(profilePath) {
    try {
      ...
      this.log(
        "warn",
        `Could not remove existing configuration: ${error.message}`,
      );
    }
  }
  ...
  async verifyConfiguration() {
    ...
    try {
      ...
      const hasConfig = content.includes(
        "Stigmergy CLI Environment Configuration",
      );
