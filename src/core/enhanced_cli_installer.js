...
class EnhancedCLIInstaller {
  ...
  async setupPermissions() {
    ...
    try {
      ...
      if (inContainer) {
        this.log(
          "info",
          "Detected container environment, using user-space installation",
        );
      ...
      if (this.isPermissionError(testResult.error)) {
        this.log(
          "warn",
          "Permission issue detected, setting up elevated context...",
        );
      ...
      this.log(
        "error",
        `Non-permission error during setup: ${testResult.error}`,
      );
  ...
  async setupUnixElevatedContext() {
    ...
    for (const tool of privilegeEscalationTools) {
      try {
        ...
        if (
          result.status === 0 ||
          (result.status !== null && result.error?.code !== "ENOENT")
        ) {
    ...
    if (availableTool) {
      return {
        ...
        note: requiresPassword
          ? "Will prompt for password per installation"
          : "Passwordless access available",
      };
    } else {
      // No privilege escalation tool found
      this.log(
        "warn",
        "No privilege escalation tool found (sudo, doas, run0, pkexec)",
      );
      this.log(
        "info",
        "Will attempt user-space installation without privileges",
      );
      return {
        success: true,
        platform: "unix",
  ...
  async installTool(toolName, toolInfo, retryCount = 0) {
    ...
    if (this.permissionMode !== "user-space") {
      ...
      if (inContainer) {
        this.log(
          "info",
          "Detected container environment, switching to user-space installation",
        );
        this.permissionMode = "user-space";
      }
    }
    ...
    try {
      ...
      if (installResult.success) {
        ...
        this.results.installations[toolName] = {
          ...
          duration:
            Date.now() -
            (this.results.installations[toolName]?.startTime || Date.now()),
      ...
      if (this.options.autoRetry && retryCount < this.options.maxRetries) {
        this.log(
          "warn",
          `Retrying installation of ${toolInfo.name} (${retryCount + 1}/${this.options.maxRetries})...`,
        );
  ...
  async executeWindowsElevatedInstallation(toolInfo) {
    ...
    try {
      this.log(
        "info",
        `Creating Windows elevated installation for: ${toolInfo.name}`,
      );

      const scriptPath = path.join(
        os.tmpdir(),
        `stigmergy-install-${Date.now()}.ps1`,
      );
      const scriptContent = `
      ...
      try {
        ...
        this.log(
          "warn",
          `Could not clean up temp script: ${cleanupError.message}`,
        );
      }
      ...
      this.log(
        "warn",
        `Elevated installation failed (${error.message}), trying user-space installation...`,
      );
      this.permissionMode = "user-space";
      return await this.executeUserSpaceInstallation(toolInfo);
    }
  ...
  async executeUnixElevatedInstallation(toolInfo) {
    ...
    if (!privilegeSetup.success) {
      this.log(
        "warn",
        "No privilege escalation tool available, using user-space installation...",
      );
      return await this.executeUserSpaceInstallation(toolInfo);
    }
    ...
    try {
      this.log(
        "info",
        `Using ${privilegeTool} for elevated installation of: ${toolInfo.name}`,
      );
      ...
      this.log(
        "warn",
        `Privilege escalation error: ${error.message}, trying user-space installation...`,
      );
      return await this.executeUserSpaceInstallation(toolInfo);
    }
  }
  ...
  async executeUserSpaceInstallation(toolInfo) {
    try {
      ...
      const installMatch = toolInfo.install.match(
        /npm\s+(?:install|upgrade)\s+(?:-g\s+)?(.+)/,
      );
      if (!installMatch) {
      ...
      this.log(
        "info",
        `Installing ${toolInfo.name} to user directory: ${userNpmDir}`,
      );
      ...
      if (result.status === 0) {
        this.log(
          "success",
          `Successfully installed ${toolInfo.name} to user directory`,
        );
        ...
        this.log(
          "info",
          "⚠️  Make sure to add the bin directory to your PATH:",
        );
        ...
        return {
          success: false,
          error: `User-space installation failed: ${errorMessage}`,
        };
      }
    } catch (error) {
      return { success: false, error: error.message };
  ...
  async executeFallbackInstallation(toolInfo) {
    ...
    try {
      ...
      if (result.status === 0) {
        ...
        const errorMessage =
          result.stderr || `Fallback failed with exit code ${result.status}`;
  ...
  async installTools(toolNames, toolInfos) {
    ...
    if (!permissionSetup.success && permissionSetup.mode !== "elevated") {
      this.log(
        "warn",
        "Permission setup failed, but proceeding with individual installations...",
      );
    }
    ...
    this.log(
      "info",
      `Installing ${totalCount} CLI tools in ${this.permissionMode} mode...`,
    );
    ...
    for (const toolName of toolNames) {
      ...
      if (!toolInfo.install) {
        this.log(
          "debug",
          `Tool ${toolName} has no install command, skipping...`,
        );
        continue;
      }
    ...
    this.log(
      "info",
      `Batch installation completed: ${successCount}/${totalCount} successful`,
    );
  ...
  async upgradeTools(toolNames, toolInfos) {
    ...
    if (!permissionSetup.success && permissionSetup.mode !== "elevated") {
      this.log(
        "warn",
        "Permission setup failed, but proceeding with individual upgrades...",
      );
    }
    ...
    this.log(
      "info",
      `Upgrading ${totalCount} CLI tools in ${this.permissionMode} mode...`,
    );
    ...
    for (const toolName of toolNames) {
      ...
      if (!originalInfo) {
        this.log(
          "warn",
          `Tool ${toolName} not found in toolInfos, skipping...`,
        );
        continue;
      }
      ...
      if (!originalInfo.install) {
        this.log(
          "debug",
          `Tool ${toolName} has no install command, skipping upgrade...`,
        );
        continue;
      }
    ...
    this.log(
      "info",
      `Batch upgrade completed: ${successCount}/${totalCount} successful`,
    );
  ...
  getInstallationSummary() {
    ...
    const successful = Object.values(installations).filter(
      (i) => i.success !== false,
    ).length;
  ...
  async executeMultiStepInstallation(toolInfo) {
    ...
    this.log(
      "info",
      `Executing multi-step installation (${steps.length} steps) for ${toolInfo.name}...`,
    );
    ...
    this.log(
      "success",
      `All ${steps.length} steps completed successfully for ${toolInfo.name}`,
    );
