...
class DirectoryPermissionManager {
  ...
  async checkWritePermission(dir = process.cwd()) {
    try {
      ...
      this.log(
        "debug",
        `Write permission check failed for ${dir}: ${error.message}`,
      );
      return false;
    }
  }
  ...
  async findWritableDirectory(startDir = process.cwd()) {
    ...
    if (await this.checkWritePermission(startDir)) {
      this.log(
        "success",
        `Current directory has write permission: ${startDir}`,
      );
    ...
    for (const dir of userDirs) {
      try {
        ...
        this.log(
          "debug",
          `Directory not accessible: ${dir} - ${error.message}`,
        );
        continue;
      }
    }
      ...
      try {
        ...
        if (await this.checkWritePermission(stigmergyDir)) {
          ...
          return {
            dir: stigmergyDir,
            created: true,
            reason: "created_directory",
          };
        }
      } catch (error) {
        this.log(
          "error",
          `Failed to create working directory: ${error.message}`,
        );
      }
    }
    ...
    try {
      const randomDir = path.join(
        tempDir,
        `stigmergy-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
      );
  ...
  async configureNpmPrefix(writableDir) {
    ...
    try {
      ...
      try {
        ...
        this.log(
          "debug",
          `npm global directory may already exist: ${error.message}`,
        );
      }
  ...
  async setupWorkingDirectory() {
    ...
    try {
      ...
      this.log(
        "info",
        `Changed working directory to: ${writableDir} (reason: ${reason})`,
      );
  ...
  generateSetupInstructions(npmConfig) {
    ...
    if (shellType === "powershell") {
      ...
      instructions.push(
        `$env:npm_config_prefix = "${npmGlobalDir.replace(/\\/g, "\\\\")}"`,
      );
      instructions.push("");
      instructions.push("# Add npm global bin to PATH");
      instructions.push(
        `$env:PATH = "${npmBinDir.replace(/\\/g, "\\\\")};$env:PATH"`,
      );
      instructions.push("");
      instructions.push("# Make persistent (add to PowerShell profile):");
      instructions.push(
        "# Run these commands manually or copy to your PowerShell profile:",
      );
      instructions.push("");
      instructions.push(
        `$env:npm_config_prefix = "${npmGlobalDir.replace(/\\/g, "\\\\")}"`,
      ...
      instructions.push(
        `# Add-Content -Path $PROFILE -Value '$env:npm_config_prefix = "${npmGlobalDir.replace(/\\/g, "\\\\")}"'`,
      );
      instructions.push(
        `# Add-Content -Path $PROFILE -Value '$env:PATH = "${npmBinDir.replace(/\\/g, "\\\\")};$env:PATH"'`,
      );
      instructions.push("");
    ...
    if (shellType === "cmd") {
      ...
      instructions.push("");
      instructions.push(
        "# Make persistent (add to system environment variables):",
      );
    ...
    if (profileFile) {
      ...
      instructions.push(
        `echo 'export npm_config_prefix="${npmGlobalDir}"' >> ${profileFile}`,
      );
      instructions.push(
        `echo 'export PATH="${npmBinDir}:$PATH"' >> ${profileFile}`,
      );
  ...
  displayResults(result) {
    if (!result.success) {
      ...
      console.log(
        "2. Try creating a project directory: mkdir ~/stigmergy && cd ~/stigmergy",
      );
