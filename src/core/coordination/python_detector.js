...
class PythonDetector extends EventEmitter {
  ...
  async detectPython() {
    try {
      ...
      if (detectionResult.found) {
        ...
        // Get version
        const versionResult = await this._getPythonVersion(
          detectionResult.executable,
        );
        ...
        const validation = await this._validatePython(
          detectionResult.executable,
        );
  ...
  _getPossiblePythonCommands() {
    ...
    if (this.options.preferredVersion) {
      const majorMinor = this.options.preferredVersion
        .split(".")
        .slice(0, 2)
        .join(".");
      commands.unshift(
        `python${majorMinor}`,
        `python${majorMinor.replace(".", "")}`,
      );
    }
  ...
  async _getPythonVersion(executable) {
    return new Promise((resolve, reject) => {
      ...
      childProcess.on("close", (code) => {
        ...
        if (code === 0) {
          try {
            const versionTuple = JSON.parse(
              stdout.trim().replace(/\(/g, "[").replace(/\)/g, "]"),
            );
