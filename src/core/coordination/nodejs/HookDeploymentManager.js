...
class HookDeploymentManager {
  ...
  async deployHooksForCLI(cliName, options = {}) {
    ...
    if (cliName.toLowerCase() === "resumesession") {
      console.log(
        `[HOOK_DEPLOYMENT] Skipping hooks deployment for ${cliName} (session recovery tool)`,
      );
      return true;
    }
  ...
  async updateHooksJson(cliName, hookDir, resumeSessionFileName) {
    ...
    if (fs.existsSync(hooksJsonPath)) {
      try {
        ...
        console.warn(
          `[HOOK_DEPLOYMENT] Failed to parse existing hooks.json, creating new one: ${error.message}`,
        );
        hooksConfig = {};
      }
    }
    ...
    console.log(
      `[HOOK_DEPLOYMENT] Updated hooks.json with resumesession command: ${hooksJsonPath}`,
    );
  }
