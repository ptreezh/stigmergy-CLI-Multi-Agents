...
class ResultAggregator {
  ...
  _votingStrategy(results) {
    ...
    for (const [output, count] of Object.entries(votes)) {
      if (count > maxVotes) {
        maxVotes = count;
        winner = successful.find(
          (r) => this._normalizeOutput(r.output) === output,
        );
      }
    }
  ...
  _consensusStrategy(results) {
    ...
    return {
      ...
      consensusRatio:
        maxCluster.totalWeight /
        successful.reduce((sum, r) => sum + (this.cliWeights[r.cli] || 1.0), 0),
    };
  }
  ...
  _mergeOutputs(results) {
    // 如果是代码，尝试选择最好的版本
    const codeBlocks = results
      .map((r) => this._extractCodeBlocks(r.output))
      .flat();
