...
class IntentRouter extends EventEmitter {
  ...
  async routeRequest(intentResult, context = {}) {
    ...
    try {
      ...
      switch (intentResult.intent) {
        ...
        case "REVIEW":
        case "OPTIMIZATION":
          routingDecision = await this._routeCrossCLICall(
            intentResult,
            context,
          );
          break;
        ...
        case "COLLABORATION":
          routingDecision = await this._routeCollaborationRequest(
            intentResult,
            context,
          );
          break;
  ...
  async _routeCrossCLICall(intentResult, context) {
    ...
    const isAvailable = await this._checkCLIAvailability(
      intentResult.targetCLI,
    );

    if (!isAvailable) {
      throw new Error(
        `Target CLI '${intentResult.targetCLI}' is not available`,
      );
    }
  ...
  async _routeCollaborationRequest(intentResult, context) {
    ...
    if (unavailableCLIs.length > 0) {
      throw new Error(
        `The following CLI tools are not available: ${unavailableCLIs.join(", ")}`,
      );
    }
  ...
  _initializeAuditLog() {
    ...
    try {
      ...
      console.error(
        `[IntentRouter] Failed to initialize audit log: ${error.message}`,
      );
    }
  }
  ...
  _auditRoutingDecision(intentResult, routingDecision, context) {
    ...
    try {
      ...
      console.error(
        `[IntentRouter] Failed to write audit log: ${error.message}`,
      );
    }
  }
    ...
    return {
      ...
      crossCLICallRate:
        this.stats.totalRequests > 0
          ? (
              (this.stats.crossCLICalls / this.stats.totalRequests) *
              100
            ).toFixed(2) + "%"
          : "0%",
      collaborationRequestRate:
        this.stats.totalRequests > 0
          ? (
              (this.stats.collaborationRequests / this.stats.totalRequests) *
              100
            ).toFixed(2) + "%"
          : "0%",
      localTaskRate:
        this.stats.totalRequests > 0
          ? ((this.stats.localTasks / this.stats.totalRequests) * 100).toFixed(
              2,
            ) + "%"
          : "0%",
      errorRate:
        this.stats.totalRequests > 0
          ? (
              (this.stats.routingErrors / this.stats.totalRequests) *
              100
            ).toFixed(2) + "%"
          : "0%",
    };
  ...
  clearAuditLog() {
    ...
    try {
      ...
      console.error(
        `[IntentRouter] Failed to clear audit log: ${error.message}`,
      );
    }
  }
}
