...
class CLIAdapterRegistry extends EventEmitter {
  ...
  async registerAdapter(adapterConfig) {
    try {
      ...
      if (
        ...
        !adapterConfig.implementation
      ) {
        throw new Error(
          "Invalid adapter configuration: missing required fields",
        );
      }
  ...
  async _initializeAdapter(adapter) {
    try {
      ...
      if (
        ...
        typeof adapter.implementation.initialize === "function"
      ) {
      ...
      throw new Error(
        `Failed to initialize adapter '${adapter.name}': ${error.message}`,
      );
    }
  }
  ...
  async _cleanupAdapter(adapter) {
    try {
      ...
      if (
        adapter.implementation &&
        typeof adapter.implementation.cleanup === "function"
      ) {
        await adapter.implementation.cleanup();
      }
    } catch (error) {
      console.error(
        `[CliAdapterRegistry] Error cleaning up adapter '${adapter.name}': ${error.message}`,
      );
    }
  }
  ...
  async _autoDiscoverAdapters() {
    ...
    try {
      ...
      if (!fs.existsSync(adapterPath)) {
        console.log(
          `[CliAdapterRegistry] Adapter base path not found: ${adapterPath}`,
        );
        return;
      }
      ...
      for (const file of files) {
        if (file.endsWith(".js") && !file.startsWith(".")) {
          ...
          try {
            ...
            console.error(
              `[CliAdapterRegistry] Error loading adapter '${adapterName}': ${error.message}`,
            );
          }
        }
      }
      ...
      console.error(
        `[CliAdapterRegistry] Error auto-discovering adapters: ${error.message}`,
      );
    }
  }
  ...
  async _performHealthChecks() {
    for (const [name, adapter] of this.state.adapters.entries()) {
      try {
        ...
        if (
          adapter.implementation &&
          typeof adapter.implementation.healthCheck === "function"
        ) {
        ...
        console.error(
          `[CliAdapterRegistry] Health check failed for adapter '${name}': ${error.message}`,
        );
      }
    }
  }
  ...
  async shutdown() {
    ...
    for (const adapterName of adapterNames) {
      try {
        ...
        console.error(
          `[CliAdapterRegistry] Error unregistering adapter '${adapterName}': ${error.message}`,
        );
      }
    }
  ...
  async reloadAdapters() {
    ...
    for (const adapterName of adapterNames) {
      try {
        ...
        if (
          adapter.implementation &&
          typeof adapter.implementation.reload === "function"
        ) {
        ...
        console.error(
          `[CliAdapterRegistry] Error reloading adapter '${adapterName}': ${error.message}`,
        );
      }
    }
  ...
  exportState() {
    return {
      adapters: Array.from(this.state.adapters.entries()).map(
        ([name, adapter]) => ({
          name: name,
  ...
  async importState(state) {
    ...
    for (const adapterData of state.adapters) {
      try {
        ...
        console.error(
          `[CliAdapterRegistry] Error importing adapter '${adapterData.name}': ${error.message}`,
        );
      }
    }
