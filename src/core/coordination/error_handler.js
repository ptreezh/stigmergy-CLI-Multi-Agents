...
class ErrorHandler extends EventEmitter {
  constructor(options = {}) {
    ...
    this.options = {
      defaultRetryPolicy:
        options.defaultRetryPolicy || RetryPolicy.EXPONENTIAL_BACKOFF,
  ...
  async handleError(error, context = {}, retryPolicy = null) {
    try {
      ...
      const handlingResult = {
        shouldRetry:
          recoveryStrategy.canRetry && policy !== RetryPolicy.NO_RETRY,
      ...
      console.error(
        "[ErrorHandler] Error while handling error:",
        handlingError,
      );
      return {
        shouldRetry: false,
  ...
  _addToHistory(errorRecord) {
    ...
    if (this.state.errorHistory.length > this.options.errorHistorySize) {
      this.state.errorHistory = this.state.errorHistory.slice(
        -this.options.errorHistorySize,
      );
    }
  }
  ...
  getStatistics() {
    return {
      ...
      retrySuccessRate:
        this.state.errorStatistics.retryStatistics.totalRetries > 0
          ? (
              (this.state.errorStatistics.retryStatistics.successfulRetries /
                this.state.errorStatistics.retryStatistics.totalRetries) *
              100
  ...
  getErrorsBySeverity(severity) {
    return this.state.errorHistory.filter(
