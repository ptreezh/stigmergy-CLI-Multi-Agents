...
class PerformanceBenchmark extends EventEmitter {
  ...
  async measureThroughput(fn, iterations = 100) {
    ...
    const throughput = {
      ...
      minResponseTime: Math.min(...results.map((r) => r.executionTime)).toFixed(
        2,
      ),
      maxResponseTime: Math.max(...results.map((r) => r.executionTime)).toFixed(
        2,
      ),
      successRate:
        ((results.filter((r) => r.success).length / iterations) * 100).toFixed(
          2,
        ) + "%",
    };
