...
class GeminiIntegrationInstaller {
  ...
  async createGeminiConfigDirectory() {
    try {
      ...
      console.log(
        `[ERROR] Failed to create Gemini config directory: ${error.message}`,
      );
      return false;
    }
  }
  ...
  async installGeminiExtensions() {
    try {
      ...
      try {
        ...
        console.log(
          `[WARN] Failed to read existing extensions config: ${error.message}`,
        );
        existingExtensions = {};
      }
      ...
      await fs.writeFile(
        GEMINI_EXTENSIONS_FILE,
        JSON.stringify(mergedExtensions, null, 2),
        "utf8",
      );

      console.log(
        `[OK] Gemini Extension configuration installed: ${GEMINI_EXTENSIONS_FILE}`,
      );
      ...
      console.log(
        `[ERROR] Failed to install Gemini Extension configuration: ${error.message}`,
      );
      return false;
    }
  }
  ...
  async copyAdapterFile() {
    try {
      ...
      try {
        ...
        console.log(
          `[WARN] Adapter source file does not exist: ${adapterSource}`,
        );
    ...
    try {
      ...
      try {
        ...
        if (removedCount > 0) {
          ...
          console.log(
            `[OK] Removed ${removedCount} cross-CLI adapters from Gemini Extensions config`,
          );
        }
      } catch (error) {
        console.log(
          `[WARN] Failed to process extensions config: ${error.message}`,
        );
      }
