...
class SmartRouter:
        ...
        for tool_name, tool_info in self.tools.items():
            ...
            for keyword in tool_info["keywords"]:
                if keyword.lower() in user_input.lower():
                    ...
                    # 检查是否需要协作 - 如果输入包含协作关键词
                    if self._needs_collaboration(clean_input):
                        # 创建协作任务
    ...
    def _create_collaboration_tasks(self, user_input, initiating_tool):
        ...
        self.project_context.add_collaboration_log(
            f"协作任务创建: {user_input} (由 {initiating_tool} 发起)",
            initiating_tool
        )
    ...
    def execute_tool(self, tool_name, args):
        ...
        try:
            ...
            task_id = f"task_{tool_name}_{int(datetime.now().timestamp())}"
            task = {{
                "id": task_id,
                "type": "execution",
            ...
            # 添加协作日志
            self.project_context.add_collaboration_log(
                f"执行任务 {task_id}: {{'成功' if result.returncode == 0 else '失败'}}",
                tool_name
            )
            ...
            # 记录异常情况
            task_id = f"task_{tool_name}_{int(datetime.now().timestamp())}_error"
            error_task = {{
                "id": task_id,
                "type": "error",
                "description": str(e),
    ...
    def _post_execution_analysis(self, result):
        ...
        if "code" in result.lower() and "test" in result.lower():
            # 需要测试智能体介入
            task_id = f"followup_test_{int(datetime.now().timestamp())}"
            task = {{
                "id": task_id,
                "type": "testing",
                "description": f"为代码生成测试: {{result[:200]}}...",
