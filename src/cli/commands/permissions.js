...
async function handlePermCheckCommand(options = {}) {
  try {
    console.log(
      chalk.cyan("[PERM-CHECK] Checking current directory permissions...\n"),
    );
    ...
    console.log(
      `🔧 Write permission: ${hasWritePermission ? chalk.green("✅ Yes") : chalk.red("❌ No")}`,
    );

    if (!hasWritePermission) {
      console.log("\n💡 Suggestions:");
      console.log(
        "1. Run: stigmergy fix-perms    # Fix permissions automatically",
      );
      console.log("2. Change to user directory: cd ~");
      console.log(
        "3. Create project directory: mkdir ~/stigmergy && cd ~/stigmergy",
      );
...
async function handleFixPermsCommand(options = {}) {
  try {
    console.log(
      chalk.cyan(
        "[FIX-PERMS] Setting up working directory with proper permissions...\n",
      ),
    );
    ...
    if (hasWritePermission) {
      console.log(
        chalk.green("✅ Current directory already has proper permissions!"),
      );
    ...
    try {
      ...
      console.log(
        chalk.green(`✅ Created Stigmergy directory: ${stigmergyDir}`),
      );
      console.log(
        chalk.yellow(
          "💡 Consider changing to this directory for your projects",
        ),
      );
      ...
      console.log(
        chalk.red("❌ Could not create directory or fix permissions"),
      );
      ...
      console.log(
        "2. Try: cd ~ && mkdir stigmergy-workspace && cd stigmergy-workspace",
      );
