...
│  │  ├── shared-context.json    (共享上下文)                │   │   │
│  │  ├── event-log.json         (事件日志)                  │   │
│  │  ├── worktree-registry.json (Worktree 注册)            │   │
│  │  └── terminal-sessions.json (终端会话)                 │   │
│  └──────────────────────────────────────────────────────────────┘   │
│                              │                                      │
│  ┌──────────────────────────────────────────────────────────────┐   │
...
ResumeSessionIntegration.restoreTaskState()
    ...
    └─ 读取历史记录
    ↓
ResumeSessionIntegration.passContextToSubtask()
    ├─ 传递项目上下文
    ├─ 传递任务描述
    ├─ 传递约束条件
    └─ 传递之前的结果
...
继续执行
    ↓
ResumeSessionIntegration.collectSubtaskContext()
    ├─ 收集子任务上下文
    ├─ 更新任务状态
    └─ 记录执行历史
    ↓
