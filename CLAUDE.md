...
**Stigmergy Cross-CLI Evolution Incubator**

把任何想法/项目/网站 → 可IM交互的专业技能包
...
Layer 1: Stigmergy 核心孵化包
  ...
  🔧 OpenCLI → 网站→CLI (453命令/73网站, v1.6.1已验证)
  🔧 CLI-Anything → 源码→CLI (7阶段流水线)
  🥚 孵化引擎 → CLI + 专业知识 → 基础技能 → 专业编排
  🧬 Soul自主进化 → 反思 → 学习 → 技能发现 → 能力提升
  🔒 Verification Gate → 硬约束拦截虚假完成声明

Layer 2: 基础操作技能层
  cli-openplc │ cli-medusa │ cli-freecad │ ...

Layer 3: 垂直专业技能层 (孵化产出)
  🏭 plc-edu │ 🛒 eb-edu │ 🏥 medical │ ⚖️ legal │ ...
```
...
- **Level 0**: 未验证
- **Level 1**: 代码/配置完成
- **Level 2**: 真实安装/部署完成 (声称"完成"的最低要求)
- **Level 3**: 真实功能测试通过
- **Level 4**: 生产环境验证

#### Requirements
1. **Real Execution**: 所有功能必须实际执行测试；禁止模拟、假设或假设性结果
2. **Complete Verification**: 测试基本功能+边界情况+错误路径+真实环境
3. **Strict Testing**: 必须经过多轮真实场景验证
4. **Honest Reporting**: 未验证内容不得声称"完成"；必须明确说明局限性
...
2. **历史错误对照** - 检测已知虚假完成模式（如：用配置完成冒充功能完成）
3. **局限性完整性** - 必须说明未验证项
...
孵化器实现自主进化系统:
- **双源记忆**: Markdown存储 + SQLite-vec向量数据库
- **自主反思**: 自我批评、错误分析、自我纠正
- **持续进化**: 技能发现、能力提升
- **知识生产**: 多Agent辩论与共识计算
...
- **CLI优先**: 任何专业系统 → 先转化为CLI → 再编排技能
- **IM统一入口**: 用户只通过IM交互，核心包隐藏所有技术复杂度
- **技能可编排**: 基础操作技能 → 可被编排为专业教学/业务技能
- **自主进化**: 孵化能力自我提升，从使用数据中学习
...
### Entry Point
- `src/index.js` - Main entry point → delegates to `src/cli/router-beta.js`
- `src/cli/router-beta.js` - Modular router with separated command handlers

### Core Layer

#### CLI Platform
- `src/core/cli_tools.js` - Supported CLI tools (Claude, Gemini, Qwen, iFlow, CodeBuddy, Copilot, Codex, Kode) with install commands, hooks dirs, OAuth config
- `src/core/cli_path_detector.js` - Detect & cache CLI tool paths
- `src/core/cli_adapters.js` - Normalize interactive vs one-time mode flags

#### Smart Routing
- `src/core/smart_router.js` - Intent analysis → CLI selection → result aggregation
- `src/core/cli_help_analyzer.js` - Analyze CLI help text for patterns/capabilities
- `src/core/execution_mode_detector.js` - Detect interactive vs one-time mode

#### Coordination (`src/core/coordination/`)
- `nodejs/HookDeploymentManager.js` - Deploy hooks to CLI tools
- `nodejs/CLIIntegrationManager.js` - Manage CLI integrations
- `nodejs/AdapterManager.js` - Adapt different CLI interfaces
- `cross_cli_executor.js` - Execute across CLIs
- `collaboration_coordinator.js` - Multi-CLI collaboration
- `intent_router.js` - Route user intents to handlers

#### Soul Engine (`src/core/soul_engine/`)
- Memory management, reflection, skill discovery
- Knowledge production through multi-agent debate
- `src/core/soul_knowledge_base.js` - Knowledge base
- `src/core/soul_manager.js` - Soul lifecycle management

### Command Handlers (`src/cli/commands/`)
- `install.js` - Install CLI tools
- `status.js` - Check status
- `scan.js` - Scan for available CLIs
- `skills.js` - Skill management
- `project.js` - Project commands (setup, deploy, init, call)
- `interactive.js` - Interactive mode controller
- `autoinstall.js` - Auto-install hooks

### Orchestration Layer (TypeScript) (`src/orchestration/`)
- `core/CentralOrchestrator.ts` - Task planning & execution
- `managers/` - GitWorktreeManager, StateLockManager, TaskPlanningFiles
- `events/EventBus.ts` - Event system
- `hooks/` - Hook system & installer
- `integration/ResumeSessionIntegration.ts` - Cross-CLI session recovery

### Gateway (`src/gateway/`)
- **cc-connect** - IM unified gateway (飞书/Telegram/微信/QQ/钉钉/Slack/Discord/LINE/...)
- Platform adapters per chat service
- Message parsing, routing, command execution, response handling

### Key Concepts

#### Incubation Flow (用户视角)

```
用户: "帮我做PLC教学系统"
孵化器:
  1. 追问: "您有源码还是网站？"
  2. 用户: "GitHub上的OpenPLC源码"
...
#### Execution Modes
- **Interactive Mode**: Execute prompt, keep CLI running for continuous conversation
- **One-Time Mode**: Execute prompt, exit (return control to caller)
- Different CLIs have different flags (handled by `cli_adapters.js`)

#### Smart Routing
- `stigmergy call "task description"` - Auto-select best CLI
- Uses pattern matching + CLI capability analysis
- Falls back to Claude if no better match

#### Cross-CLI Communication
```bash
stigmergy <tool_name> <task_description>
```
- Uses hooks deployed to each CLI's hooks directory
- OAuth authentication for tools like Qwen
...
- `tsconfig.build.json` - Build config for orchestration layer only → `dist/orchestration/`
- `npm run build:orchestration` builds only `src/orchestration/**/*`

### Available CLI Tools

Supported AI CLI tools (`src/core/cli_tools.js`):
Claude · Gemini · Qwen · iFlow · QoderCLI · CodeBuddy · Copilot · Codex · Kode

### Skills System

Stigmergy integrates with OpenSkills:
- `src/core/skills/StigmergySkillManager.js` - Unified skill manager
- `src/core/skills/embedded-openskills/` - OpenSkills core
- `src/commands/skill.js` - Skill command handler
- Skills installable from GitHub repositories
...
Core: chalk · commander · inquirer · js-yaml · semver · axios · playwright
**Node.js >= 16.0.0**
...
|-------|-------------|------|
| resumesession | Cross-CLI session recovery and history management skill | `.claude/skills/resumesession/SKILL.md` |
<!-- GSD:skills-end -->
