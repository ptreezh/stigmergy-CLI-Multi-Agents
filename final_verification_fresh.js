// Final verification test for all CLI tools after fixes (force fresh analysis)
const CLIHelpAnalyzer = require('./src/core/cli_help_analyzer');
const { CLI_TOOLS } = require('./src/core/cli_tools');

async function finalVerification() {
  console.log('Final verification of all CLI tools after fixes (force fresh analysis)...\n');
  ...
  for (const [toolName, toolConfig] of Object.entries(CLI_TOOLS)) {
    console.log(`=== Verifying ${toolName} (fresh analysis) ===`);
    ...
    try {
      ...
      const patterns = analyzer.extractPatterns(helpInfo.rawHelp, cliType, toolName);
