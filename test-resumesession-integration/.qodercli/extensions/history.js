// QoderCLI ResumeSession Integration
...
class SessionScanner {
  scanSessions(cliType, sessionsPath, projectPath) {
    ...
    try {
      ...
      // For QoderCLI: scan projects/<projectName>/
      if (cliType === 'qodercli' && sessionsPath.includes('projects')) {
        ...
        if (file.endsWith('.jsonl') || file.endsWith('.json')) {
          try {
            ...
            if (file.endsWith('.jsonl')) {
              ...
              if (firstUserMsg) {
                let msgContent = firstUserMsg.content || firstUserMsg.message || firstUserMsg.text;
                if (typeof msgContent === 'object') {
                  msgContent = JSON.stringify(msgContent);
                }
                if (typeof msgContent === 'string' && msgContent.trim()) {
                  title = msgContent.substring(0, 100) || title;
                }
              }
            ...
            // Handle JSON format
              ...
              sessionData = {
                sessionId: data.id || data.sessionId || file.replace('.json', ''),
                title: data.title || data.topic || 'Untitled',
                messages: data.messages || [],
                timestamp: data.updatedAt || data.timestamp || fs.statSync(filePath).mtime,
                cwd: data.projectPath || data.workingDirectory,
                projectPath: data.projectPath || data.workingDirectory,
...
const qoderHistoryHandler = new QoderCLIHistoryHandler();

const qoderHistory = {
  name: 'history',
  handler: qoderHistoryHandler.handler.bind(qoderHistoryHandler)
};
