...
class HistoryFormatter {
  formatSummary(sessions, context) {
    ...
    response += `💡 **使用方法:**\n`;
    response += `• '{{COMMAND_NAME}} --cli <工具>' - 查看特定CLI\n`;
    response += `• '{{COMMAND_NAME}} --search <关键词>' - 搜索内容\n`;
    response += `• '{{COMMAND_NAME}} --format timeline' - 时间线视图`;
...
async function handleHistoryCommand(input, context) {
  try {
    ...
    const result = historyQuery.queryHistory(query, 'D:\stigmergy-CLI-Multi-Agents\test-resumesession-integration');
...
if (typeof global !== 'undefined' && global.claudeCLI) {
  global.claudeCLI.addSlashCommand('{{COMMAND_NAME}}', handleHistoryCommand);
}
