      ...
      for (const file of files) {
        if (file.endsWith('.jsonl')) {
          try {
            ...
            if (firstUserMsg && firstUserMsg.message && firstUserMsg.message.parts && firstUserMsg.message.parts[0]) {
              title = firstUserMsg.message.parts[0].text?.substring(0, 100) || title;
            }
      ...
      return sessionData.messages
        .map(msg => {
          if (msg.message && msg.message.parts) {
            return msg.message.parts.map(p => p.text || '').join(' ');
          }
          return '';
        })
        .join(' ');
