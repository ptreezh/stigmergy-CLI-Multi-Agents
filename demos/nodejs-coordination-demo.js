...
async function demo() {
  ...
  if (initialized) {
    ...
    console.log('- Health:', status.health.healthy ? 'âœ?Healthy' : 'â?Unhealthy');
    console.log('- Cross-CLI calls:', status.statistics.counters.cross_cli_calls);
    
    console.log('\n');
  } else {
    console.log('â?Failed to initialize coordination layer\n');
  }
  ...
  if (hookDeployed) {
    ...
    console.log('â?Failed to deploy hooks\n');
  }
