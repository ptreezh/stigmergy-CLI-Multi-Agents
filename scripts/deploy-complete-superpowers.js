...
async function sessionStart(context) {
  try {
    ...
    console.log('🦸 Superpowers context injected successfully for ${cliConfig.name}'.replace('\${cliConfig.name}', '${cliConfig.name}'));
  } catch (error) {
    // Silently fail to not break the session
    try { console.error('Failed to inject superpowers context:', error.message); } catch (e) {}
  }
...
`;
  ...
  function generateHooksConfig(cliName, cliConfig) {
    const homeVar = cliConfig.home.toUpperCase().replace(/\./g, "_");
    return {
      hooks: {
        sessionStart: {
          name: "SessionStart",
