...
function showWelcome() {
  console.log('');
  log(colors.cyan, '╔════════════════════════════════════════════════════════════╗');
  log(colors.cyan, '║' + colors.bright + '   Stigmergy 自主进化系统启动' + colors.reset + colors.cyan + '                              ║');
  log(colors.cyan, '║' + colors.bright + '   Stigmergy Autonomous Evolution System' + colors.reset + colors.cyan + '                    ║');
  log(colors.cyan, '╚════════════════════════════════════════════════════════════╝');
  console.log('');
}
...
async function showSystemStatus() {
  ...
  if (fs.existsSync(memoryFile)) {
    ...
    const sessionCount = content.trim().split('\n').filter(l => l).length;
    log(colors.green, `\n  💾 会话记忆: ${sessionCount} 条`);
  }
...
function showEvolutionPlan() {
  ...
  const phases = [
    { stage: '阶段 1', name: '双 Agent Loop 基础架构', weeks: '2-3 周', priority: '🔴 高' },
    { stage: '阶段 2', name: '自我评估系统', weeks: '3-4 周', priority: '🔴 高' },
    { stage: '阶段 3', name: '闭环进化系统', weeks: '4-6 周', priority: '🔴 高' },
    { stage: '阶段 4', name: '自主学习系统', weeks: '6-8 周', priority: '🟡 中' },
    { stage: '阶段 5', name: '长时运行系统', weeks: '4-6 周', priority: '🟡 中' },
  ...
  for (const phase of phases) {
    console.log(`  ${phase.stage.padEnd(6)} ${phase.name.padEnd(22)} ${phase.weeks.padEnd(9)} ${phase.priority}`);
  }
