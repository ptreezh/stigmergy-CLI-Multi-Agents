...
REM 检查参数
if "%~1"=="" (
    echo 用法: codebuddy-call ^<cli_name^> [arguments...]
    echo 示例: codebuddy-call claude --version
    echo.
    echo 已安装的CLI工具:
...
if /i "%CLI_NAME%"=="gemini" (
    echo 调用 Gemini CLI: gemini %ARGS%
    echo --------------------------------------------------
    gemini %ARGS%
    exit /b %ERRORLEVEL%
)
...
echo 请使用以下工具之一: claude, gemini, qwen, iflow, codex, copilot
exit /b 1