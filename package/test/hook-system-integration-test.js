...
function testHookInstallation() {
    ...
    for (const hookDir of hookDirectories) {
        ...
        console.log(`${exists ? 'âœ? : 'â?} ${hookDir}: ${exists ? 'Found' : 'Missing'}`);
    }
...
function testHookContentVerification() {
    ...
    for (const hookFile of hookFiles) {
        if (fs.existsSync(hookFile.path)) {
            ...
            console.log(`${hasRequiredContent ? 'âœ? : 'â?} ${hookFile.path}: ${hasRequiredContent ? 'Valid' : 'Invalid content'}`);
            if (hasRequiredContent) validHooks++;
        } else {
            console.log(`â?${hookFile.path}: File not found`);
        }
    }
...
function testHookExecution() {
    ...
    try {
        ...
        const testScenarios = [
            {
                tool: 'claude',
                input: 'è¯·åˆ†æžè¿™ä¸ªReactç»„ä»¶çš„å®‰å…¨æ€?,
        ...
        for (const scenario of testScenarios) {
            ...
            try {
                ...
                if (result.detected && result.skill === scenario.expectedSkill) {
                    ...
                    console.log(`â?Hook failed to detect skill (expected: ${scenario.expectedSkill})`);
                }
            } catch (error) {
                console.log(`â?Hook execution error: ${error.message}`);
            }
        }
    ...
    for (const file of filesToCheck) {
        if (fs.existsSync(file)) {
            ...
            if (!hasNonAnsi) {
                ...
                console.log(`â?${file}: Contains non-ANSI characters`);
...
async function testRealAIToolCalling() {
    ...
    try {
        ...
        const testCalls = [
            {
                tool: 'claude',
                command: 'è¯·å°†ä»¥ä¸‹æ–‡æœ¬ç¿»è¯‘æˆè‹±æ–‡ï¼šHello World',
                expectedSuccess: true
            },
            {
                tool: 'gemini',
                command: 'åˆ†æžè¿™ä¸ªå‡½æ•°çš„å¤æ‚åº¦',
                expectedSuccess: true
            }
        ];
        ...
        for (const test of testCalls) {
            ...
            try {
                ...
                if (result.success === test.expectedSuccess) {
                    ...
                    console.log(`â?${test.tool} execution failed (expected: ${test.expectedSuccess})`);
                }
            } catch (error) {
                console.log(`â?${test.tool} execution error: ${error.message}`);
            }
        }
        ...
        console.log(`â?Real AI tool calling test failed: ${error.message}`);
        return false;
    }
}
...
function testCrossPlatformCompatibility() {
    ...
    for (const testPath of testPaths) {
        ...
        console.log(`${works ? 'âœ? : 'â?} ${testPath}: ${works ? 'Valid' : 'Invalid path'}`);
        if (works) validPaths++;
    }
