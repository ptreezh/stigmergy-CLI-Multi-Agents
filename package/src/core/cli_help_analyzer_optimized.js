// Enhanced CLI Help Analyzer with better pattern extraction and optimized error handling
...
class CLIHelpAnalyzer {
   ...
   * Optimized to reduce error messages during initialization
   */
  async initialize() {
    try {
      ...
      if (!configExists) {
        ...
        try {
          ...
          if (!createError.message.includes('EACCES') && !createError.message.includes('EPERM')) {
            await errorHandler.logError(createError, 'ERROR', 'CLIHelpAnalyzer.initialize');
          }
        }
      }
  ...
  async getHelpInfo(cliName, cliConfig) {
    ...
    for (const helpArgs of helpMethods) {
      try {
        ...
        // Try next method - don't log error for expected "command not found" cases
        continue;
      }
    }
  ...
  extractPatterns(helpText, cliType) {
    ...
    // Enhanced pattern extraction for better parameter handling
  ...
  /**
   * Cache analysis results with optimized error handling
   */
  async cacheAnalysis(cliName, analysis) {
    try {
      ...
      // Only show cache errors in debug mode
      if (process.env.DEBUG === 'true') {
  ...
  async getCachedAnalysis(cliName) {
    try {
      ...
      // Don't log cache read errors unless in debug mode
      if (process.env.DEBUG === 'true') {
        console.error(`Error reading cache for ${cliName}:`, error.message);
      }
      return null;
    }
  }
  ...
  isCacheExpired(timestamp) {
    ...
    return hoursDiff > 24; // 24 hour cache expiration
  }

  /**
   * Record failed analysis attempt with optimized error handling
   */
  ...
  async loadPersistentConfig() {
    try {
      ...
      // Don't show file not found errors - they're expected on first run
      if (process.env.DEBUG === 'true' && !error.message.includes('ENOENT')) {
  ...
  async savePersistentConfig(config) {
    try {
      ...
      // Don't spam error messages when saving config fails
      if (process.env.DEBUG === 'true') {
  ...
  /**
   * Get CLI pattern for specific tool with optimized error handling
   */
