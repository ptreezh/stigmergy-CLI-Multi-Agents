...
class SmartRouter {
  ...
  async getOptimizedCLIPattern(toolName) {
    try {
      ...
      // Only re-analyze if we have a recent failure record or cache is expired
      const config = await this.analyzer.loadPersistentConfig();
      const failedAttempt = config.failedAttempts[toolName];
      
      // If there was a recent failure (less than 1 hour ago), skip re-analysis
      if (failedAttempt && this.isRecentFailure(failedAttempt.timestamp)) {
        console.log(`[INFO] Skipping re-analysis for ${toolName} due to recent failure`);
        return cached || null;
      }
