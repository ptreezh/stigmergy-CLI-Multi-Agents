...
测试详情:
✓ should route to claude when claude keyword is detected (15ms)
✓ should route to qwen when qwen keyword is detected (12ms)
✓ should route to default tool when no keyword is detected (10ms)
...
```

### 8.2 集成测试报告
```
测试套件: Routing Integration
总测试数: 8
通过数: 8
失败数: 0
跳过数: 0
...
测试详情:
✓ should successfully route and execute CLI command (120ms)
✓ should handle CLI execution errors gracefully (85ms)
...
```
