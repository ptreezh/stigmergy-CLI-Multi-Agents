...
if "%INPUT%"=="" (
    echo 🚀 {tool_name} 简化路由器
    echo 💡 用法: {tool_name}_smart "用其他工具帮我..."
    exit /b
)
...
def smart_route():
    if len(sys.argv) < 2:
        print("🚀 {tool_name} 简化路由器")
        print("💡 用法: python {tool_name}_smart.py \\"用其他工具帮我...\\"")
        return
    ...
    # 默认执行原工具
    try:
        subprocess.run(["{tool_name}", user_input])
    except FileNotFoundError:
