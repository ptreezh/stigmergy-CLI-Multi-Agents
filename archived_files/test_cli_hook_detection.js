...
async function testCLICrossCLIDetection() {
  ...
  try {
    ...
    for (const testCase of testCases) {
      ...
      if (!hook) {
        console.log(`✗ Hook for ${testCase.cli} not found`);
        continue;
      }
      ...
      if (result && result.includes('Simulated cross-CLI call')) {
        console.log(`✓ ${testCase.cli} hook correctly detected cross-CLI request`);
        console.log(`  Prompt: "${testCase.prompt}"`);
        console.log(`  Result: "${result}"`);
        
        // Check if the correct target CLI was detected
        if (result.includes(testCase.expectedTarget)) {
          console.log(`✓ ${testCase.cli} hook correctly identified target CLI: ${testCase.expectedTarget}`);
        } else {
          console.log(`✗ ${testCase.cli} hook identified wrong target CLI`);
        }
      } else {
        console.log(`✗ ${testCase.cli} hook failed to detect cross-CLI request`);
        console.log(`  Prompt: "${testCase.prompt}"`);
        console.log(`  Result: "${result}"`);
      }
    ...
    for (const testCase of nonCrossCLIPrompts) {
      ...
      if (result === null) {
        console.log(`✓ ${testCase.cli} hook correctly ignored non-cross-CLI prompt: "${testCase.prompt}"`);
      } else {
        console.log(`✗ ${testCase.cli} hook incorrectly processed non-cross-CLI prompt: "${testCase.prompt}"`);
      }
    }
