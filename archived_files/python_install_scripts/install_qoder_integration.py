...
'''
    ...
    def _create_global_cross_cli_documentation(self) -> None:
        """创建全局Cross-CLI通信文档"""
        try:
            doc_content = """# Qoder CLI 跨CLI通信指南
...
"""
            ...
            doc_path = os.path.join(self.qoder_config_dir, "CROSS_CLI_GUIDE.md")
            with open(doc_path, 'w', encoding='utf-8') as f:
                f.write(doc_content)
                
            logger.info(f"创建Cross-CLI通信指南: {doc_path}")
            
            # 如果存在qoder.md文件，则在末尾追加Cross-CLI通信提示
            qoder_md_path = os.path.join(os.path.expanduser("~/.qoder"), "qoder.md")
            if os.path.exists(qoder_md_path):
                cross_cli_content = """
...
可用工具：claude, gemini, qwen, iflow, qodercli, codebuddy, copilot, codex
"""
                with open(qoder_md_path, 'a', encoding='utf-8') as f:
                    f.write(cross_cli_content)
                logger.info(f"在QODER.md末尾追加Cross-CLI通信提示")
                
        except Exception as e:
            logger.warning(f"创建Cross-CLI通信指南失败: {e}")
