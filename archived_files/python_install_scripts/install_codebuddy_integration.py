...
def create_global_cross_cli_documentation(config_dir):
    """创建全局Cross-CLI通信文档"""
    try:
        doc_content = """# CodeBuddy CLI 跨CLI通信指南
...
"""
        ...
        codebuddy_md_path = os.path.join(os.path.expanduser("~/.codebuddy"), "codebuddy.md")
        if os.path.exists(codebuddy_md_path):
            cross_cli_content = """
...
可用工具：claude, gemini, qwen, iflow, qodercli, codebuddy, copilot, codex
"""
            with open(codebuddy_md_path, 'a', encoding='utf-8') as f:
                f.write(cross_cli_content)
            print("[OK] 在CODEBUDDY.md末尾追加Cross-CLI通信提示")
